...
const Navbar = () => {
    ...
    return (
        ...
        <>
            {/* คอม */}
            <nav className=' bg-[#FAF9F6] hidden text-lg container mx-auto md:flex justify-between py-6 sticky top-0'>
                ...
                <ul className='flex justify-between w-[15%]'>
                    <NavLink to='/' className={({ isActive }) => (isActive ? 'font-semibold' : undefined)}><li className='hover-underline-animation left'>Home</li></NavLink>
            ...
            {/* มือถือ */}
            <nav className='bg-[#FAF9F6] drop-shadow-md md:hidden sticky top-0 py-5 px-4 flex items-center justify-between'>
                <h1 className='font-semibold text-2xl'>Chanathip</h1>
            ...
            <nav className={`${isNav ? "opacity-100 max-h-52 translate-y-0" : "translate-y-5 max-h-0 opacity-0"} md:hidden transition-all overflow-hidden bg-[#FAF9F6] flex flex-col text-lg container mx-auto justify-between sticky top-12 drop-shadow-md`}>
